...
function setupAITools() {
    ...
    if (grammarBtn) {
        grammarBtn.addEventListener('click', () => {
            ...
            grammarBtn.innerHTML = '<span>🤖</span><span>Analyzing...</span>';
            
            setTimeout(() => {
                document.getElementById('grammarResults').classList.remove('hidden');
                grammarBtn.disabled = false;
                grammarBtn.innerHTML = '<span>🤖</span><span>Analyze</span>';
    ...
    const subjectBtn = document.getElementById('analyzeSubjectBtn');
    if (subjectBtn) {
        subjectBtn.addEventListener('click', () => {
            const input = document.getElementById('subjectInput').value;
            if (!input) {
                showNotification('Please enter a subject line first', 'warning');
                return;
            }
            
            // Simulate AI processing
            subjectBtn.disabled = true;
            subjectBtn.innerHTML = '<span>🤖</span><span>Analyzing...</span>';
            
            setTimeout(() => {
                document.getElementById('subjectResults').classList.remove('hidden');
                subjectBtn.disabled = false;
                subjectBtn.innerHTML = '<span>🤖</span><span>Analyze</span>';
                showNotification('Subject line analyzed!', 'success');
            }, 1500);
        });
    }
    
    // Spam checker
    const spamBtn = document.getElementById('checkSpamBtn');
    if (spamBtn) {
        spamBtn.addEventListener('click', () => {
            const input = document.getElementById('spamInput').value;
            if (!input) {
                showNotification('Please enter email content first', 'warning');
                return;
            }
            
            // Simulate AI processing
            spamBtn.disabled = true;
            spamBtn.innerHTML = '<span>🤖</span><span>Checking...</span>';
            
            setTimeout(() => {
                document.getElementById('spamResults').classList.remove('hidden');
                spamBtn.disabled = false;
                spamBtn.innerHTML = '<span>🤖</span><span>Check Spam Score</span>';
                showNotification('Spam check complete!', 'success');
            }, 1500);
        });
    }
    
    // Name extractor
    const nameBtn = document.getElementById('extractNamesBtn');
    if (nameBtn) {
        nameBtn.addEventListener('click', () => {
            const input = document.getElementById('nameInput').value;
            if (!input) {
                showNotification('Please enter email addresses first', 'warning');
                return;
            }
            
            // Simulate AI processing
            nameBtn.disabled = true;
            nameBtn.innerHTML = '<span>🤖</span><span>Extracting...</span>';
            
            setTimeout(() => {
                document.getElementById('nameResults').classList.remove('hidden');
                nameBtn.disabled = false;
                nameBtn.innerHTML = '<span>🤖</span><span>Extract Names</span>';
                showNotification('Names extracted successfully!', 'success');
            }, 1500);
        });
    }
}
